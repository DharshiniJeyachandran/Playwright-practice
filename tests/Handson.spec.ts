...
test.only("Amazon validations", async ({ page }) => {
  ...
  const all = await page.getByRole("button", {name: "Open All Categories Menu"});
  await all.click();
  ...
  const bestSeller = await page.getByRole("heading", {name : "Amazon Best Sellers"});
  ...
  await expect(page.getByRole("heading", {name :"Best Sellers in Beauty"})).toBeVisible();
  const premiumBeauty = await page.getByRole("link", {name: "Premium Beauty"});
  await premiumBeauty.click();
  
  await expect(page.getByRole("img", {name: "Premium Beauty"})).toBeVisible();
  await expect(page.getByRole("img", {name: "Kerastase", exact:true})).toBeVisible();
